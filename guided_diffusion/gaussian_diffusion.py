...
class GaussianDiffusion:
    ...
    def p_sample(
        ...
        sample = out["mean"] + nonzero_image * th.exp(0.5 * out["log_variance"]) * noise
           ...
           for i in indices:
                ...
                with th.no_grad():
                    if img.shape != (1, 2, 224, 224):
                        img = torch.cat((org_MRI,img), dim=1)       #in every step, make sure to concatenate the original image to the sampled translation img
                    out = self.p_sample(
                        model,
                        img.float(),
                        t,
    ...
    def ddim_sample(
        ...
        sample = mean_pred + nonzero_img * sigma * noise
        return {"sample": sample, "pred_xstart": out["pred_xstart"]}
    ...
    def training_losses_translation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        """
        ...
        res_t = self.q_sample(res, t, noise=noise)     #add noise to the translation channel
        x_t=x_start.float()
        x_t[:, -1:, ...]=res_t.float()
        terms = {}
